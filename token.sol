...
contract Cumstar is Context, IERC20, Ownable {
    ...
    uint256 public ContractCreationTimeInSeconds = now; // set the seconds that the contract was created...
    uint256 public antiDumpSell = 60; // time in seconds
    uint256 public raiseSellingTaxFor = 2592000; // time in seconds
    ...
    bool public enableAntiDump = false;
    ...
    constructor () public {
        ...
        _isExcludedFromFee[0x2D045410f002A95EFcEE67759A92518fA3FcE677] = true; // dxlocker
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // if you buy when the anti dump is enabled you get awarded with a timestamp 
        if (from == uniswapV2Pair && to != uniswapV2Pair && enableAntiDump) {
        	sellCooldown[from] = now;
        }

        // if you try and sell and you have a timestamp, the sale has to be after the cooldown period - set at 60 seconds, but is changeable
        if (from != uniswapV2Pair && to == uniswapV2Pair && enableAntiDump) {
        	require(sellCooldown[from].add(antiDumpSell) < now, "Please wait before selling");
        }

        // enable the 30% selling tax for everyone !! no one is spared
        if (to == uniswapV2Pair && ContractCreationTimeInSeconds.add(raiseSellingTaxFor) > now) {
        	_liquidityFee = 15;
        	_taxFee = 15;
        	_developmentFee = 0;
        	_platformFee = 0;
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(platformAddress, transferredBalance.div(_liquidityFee).mul(_platformFee));
        transferToAddressETH(DevelopmentAddress, transferredBalance.div(_liquidityFee).mul(_developmentFee));
    }
    ...
    function setDevelopmentFee(uint256 divisor) external onlyOwner() {
        _developmentFee = divisor;
    }
    
    function setPlatformFee(uint256 divisor) external onlyOwner() {
        _platformFee = divisor;
    }

    function SetenableAntiDump(bool _enabled) public onlyOwner() { 
    	enableAntiDump = _enabled;
    }
    ...
    function setPlatformAddress(address _platform) external onlyOwner() {
        platformAddress = payable(_platform);
    }

    function setDevelopmentAddress(address _development) external onlyOwner() {
        DevelopmentAddress = payable(_development);
    }
    ...
    function afterPreSale() external onlyOwner() {
        ...
        // reset the contract creation time to reset the timer for the anti dump...
        ContractCreationTimeInSeconds = now;
        enableAntiDump = true;
    }
